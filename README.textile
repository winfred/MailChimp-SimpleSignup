...
* hosted signup form fallback

h1. TODO:
* make unit tests not collide again... currently must be run one at a time. =(
* un-hardcode the admin account flag - will be going with a futon edit process for giving an account admin access to maintain security
* update readme to include mongodb logging info - probably wont fully switch even though it's a better "tool for the job". The job is pretty easy so either couch or mongo will work for user accounts.
